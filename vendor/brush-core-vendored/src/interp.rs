...
impl ExecutionParameters {
    ...
    /// Assigns a cancellation token to this execution context.
    ///
    /// # Arguments
    ///
    /// * `token` - The cancellation token to associate with this context.
    pub fn set_cancel_token(&mut self, token: CancellationToken) {
        self.cancel_token = Some(token);
    }

    /// Returns the cancellation token associated with this execution context,
    /// if there is one.
...
/// Returns an interruption error if cancellation has been requested.
    ...
    async fn execute(
        ...
        let join_handle = tokio::spawn(async move {
            ...
            let spawn_result = body
                ...
                .await?;
            match spawn_result.wait(cancel_token).await? {
