...
/// Interval between cancellation checks while waiting for input.
...
impl<F: Fn() -> bool> InputReader<F> {
    ...
    fn read_event(&mut self) -> Result<InputEvent, brush_core::Error> {
        loop {
            // Give up promptly if cancellation has been requested.
            if (self.is_cancelled)() {
                return Err(ErrorKind::Interrupted.into());
            }

            // Check timeout before attempting read.
            let remaining = if let Some(deadline) = self.deadline {
            ...
            if cfg!(unix) {
                // Wait for input in bounded slices so cancellation is noticed
                // promptly, even when no timeout was requested.
                let slice = remaining.map_or(CANCELLATION_POLL_INTERVAL, |remaining| {
                    remaining.min(CANCELLATION_POLL_INTERVAL)
                });
                match brush_core::sys::poll::poll_for_input(&self.input, slice) {
                    Ok(true) => break, /* Data available, proceed. */
                    Ok(false) => {}    /* Interval elapsed; loop to re-check. */
...
fn read_line_with_reader(
    reader: &mut InputReader<impl Fn() -> bool>,
...
impl ReadCommand {
    ...
    fn read_line(
        ...
        let mut reader = InputReader::new(input_file, timeout, term_mode, is_cancelled);
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn test_read_event_returns_interrupted_when_cancelled() {
        let pipe = std::io::pipe();
        assert!(pipe.is_ok(), "pipe creation must succeed");
        let Ok((reader, _writer)) = pipe else {
            unreachable!("asserted successful pipe creation above");
        };

        let input = brush_core::openfiles::OpenFile::from(reader);
        let mut input_reader = InputReader::new(input, None, None, || true);

        let result = input_reader.read_event();
        assert!(result.is_err(), "cancelled state must interrupt read");
    ...
    #[cfg(unix)]
    #[test]
    fn test_read_event_reads_available_input_when_not_cancelled() {
        use std::io::Write as _;

        let pipe = std::io::pipe();
        assert!(pipe.is_ok(), "pipe creation must succeed");
        let Ok((reader, mut writer)) = pipe else {
            unreachable!("asserted successful pipe creation above");
        };
        assert!(writer.write_all(b"x").is_ok());

        let input = brush_core::openfiles::OpenFile::from(reader);
        let mut input_reader = InputReader::new(input, None, None, || false);

        let event = input_reader.read_event();
        assert!(
            matches!(event, Ok(InputEvent::Char('x'))),
            "available input must be read when not cancelled"
        );
    }
}
